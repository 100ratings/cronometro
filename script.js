document.addEventListener('DOMContentLoaded', () => {
    ...
    // Fullscreen (apenas Android, como no original)
    ...
    // “Arredonda” para que MMSSCC seja múltiplo de 9
    function snapToNearestMultipleOfNine(timeInMillis) {
        ...
        if (remainder === 0) {
            ...
            targetNumber = completeNumber - remainder;
        } else {
            targetNumber = completeNumber + (9 - remainder);
        }
        ...
        const adjustedMillis = (targetMinutes * 60000) + (targetSeconds * 1000) + (targetCentiseconds * 10);
    ...
    resetLapBtn.addEventListener('click', () => {
        if (isRunning) {
            // Placeholder de Volta (continua não implementado)
    ...
    if ('serviceWorker' in navigator) {
        window.addEventListener('load', () => {
            // Use caminho relativo para funcionar no GitHub Pages (subpasta)
